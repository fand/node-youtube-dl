...
/**
 *pp Calls youtube-dl with some arguments and the `callback`
...
ytdl.getInfo = function(url, args, options, callback) {
  ...
  call(url, defaultArgs, args, options, function(err, _data) {
    if (err) return callback(err);

    // Parse JSONs.
    var data = (Array.isArray(_data))? _data : [_data];
    var playlist = [];
    for (var i=0; i<data.length; i++) {
      if (isJSON(data[i])) {
        playlist.push(filterData(JSON.parse(data[i])));
      }
    }
